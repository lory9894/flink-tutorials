...
public class ListStateExample {
        ...
        SingleOutputStreamOperator<Tuple2<String, Integer>> filteredStream = dataStream.
                map(new MapFunction<String, Tuple2<String, Integer>>() { // Map to Tuple2, where f0 is the name and f1 is the count (number of occurrences)
                    ...
                    public Tuple2<String, Integer> map(String value) throws Exception {
                        ...
                        // (could be condensed to the new flatMap, but in that case I would not have a keyBy and I would not be able to use the stateValue)
                    }
    ...
    static class StatefullMap extends RichFlatMapFunction<Tuple2<String, Integer>, Tuple2<String, Integer>> {
         ...
         private transient ListState<Integer> elemList;


        @Override
        public void flatMap(Tuple2<String, Integer> value, Collector<Tuple2<String, Integer>> out) throws Exception {

            int countValue = count.value() == null ? 0 : count.value();


            countValue++;
            elemList.add(value.f1);

            count.update(countValue);

            if (countValue >= 5){ //when count hits 5, we collect the sum and reset the count, so we can start counting again

                int sum = 0;
                for( Integer element : elemList.get()){ // for every element in the list, we add it to the sum
                    sum += element;
                }
        ...
        public void open(org.apache.flink.configuration.Configuration conf) {
            ...
            elemList = getRuntimeContext().getListState(new ListStateDescriptor<Integer>("elemList", Integer.class)); // evidently, list have a default value of "new List"
